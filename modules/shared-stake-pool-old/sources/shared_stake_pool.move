...
// The problem with this is that people whose stake is in pending_inactive state will
// continue to accrue interest, and this sytem will account for that. However, the
// unlock function will unlock days ahead of that. I.e., you can unlock 10 APT,
// and 10 APT will be avaiable in 14 days, however in that time you earned 1 more APT
// so now you are owed 11 APT, but inactive only has 10 APT for you to claim.
// The other APT is not lost, it's just in 'active' instead of inactive.
// I think the best way to do this would be to move more stake from active to inactive
// per epoch to account for this, and then deliberately use the value from the
// epoch prior to the unlock, so unstakers only lose out on 1 epoch worth of gains.
...
module openrails::shared_stake_pool {
    ...
    public entry fun withdraw_to_coins(account: &signer, this: address, amount: u64): Coin<AptosCoin> acquires EpochTracker, SharedStakePool {
        ...
        if (stake_pool.validator_status == VALIDATOR_STATUS_INACTIVE) {
            ...
            if (timestamp::now_seconds() >= epoch_tracker.locked_until_secs) {
                ...
                if (amount > inactive) {
                    unlock(account, this, amount - inactive);
                };
            };
        };
    ...
    fun move_pending_inactive_to_inactive(stake_pool: &mut SharedStakePool) {
        ...
        while (i < len) {
            let addr = vector::borrow(&addresses, i);
            let pending_balance = simple_map::borrow(&pending_inactive_map, addr);
